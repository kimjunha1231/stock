...
type StackTab = 'all' | 'frontend' | 'backend' | 'data' | 'infra' | 'ai';
...
const stacks: Record<Exclude<StackTab, 'all'>, { label: string; summary: string; items: [string, string, string][] }> = {
  frontend: {
    ...
    items: [
      ...
      ['TanStack Table', '대용량 표', '계열사·채널·상품·로트·예약 capacity 필터/정렬/페이지네이션'],
  ...
  backend: {
    ...
    items: [
      ['Java 17', '런타임', '현재 프로젝트 기준 엔터프라이즈 JVM 버전'],
      ['Spring Boot 3.5.16', 'API·애플리케이션', '계열사·상품·재고·전략·승인 도메인 구성'],
      ['Spring Security + JWT', '인증·인가', '계열사·채널·역할 scope를 서버에서 검증'],
      ['Spring Batch', '정기 처리', '재고 갱신·위험 분석·판매 결과 집계와 재실행 관리'],
      ['Flyway', '스키마 변경', 'Oracle 스키마를 버전과 마이그레이션으로 추적'],
      ['MyBatis', '데이터 접근', '공통 상품키·로트·예약·재고·이력 조회 SQL을 명시적으로 관리'],
      ['JUnit', '단위·통합 테스트', '권한·계산·상태 전이·배치 멱등성 검증'],
    ],
  },
  data: {
    label: '데이터',
  ...
  infra: {
    ...
    items: [
      ['Docker + Docker Compose', '실행 환경', '개발·테스트·운영 구성 차이를 줄이는 컨테이너화'],
      ['Jenkins + Git/GitHub', 'CI/CD', '테스트·빌드·배포와 변경 이력 관리'],
      ['Nginx', '웹 진입점', '정적 자산·API 라우팅·TLS 종료·기본 보호'],
      ['AWS S3', '파일·증빙', '첨부·배치 산출물·감사 증빙을 비공개 저장'],
      ['k6', '부하 테스트', '대시보드·시뮬레이션·배치 API의 성능 기준 검증'],
      ['Microsoft Teams API', '승인 알림', '승인 요청·결과와 서비스 링크를 전달하며 기록은 서비스에 남김'],
    ],
  },
  ai: {
    label: 'AI·모니터링',
    ...
    items: [
      ['Spring AI', 'AI adapter', '모델 공급자 변경을 격리하고 전략 설명·요약을 호출'],
      ['LLM 모델', '미정·선정 필요', '품질·비용·보존·학습 사용·리전 정책 검토 후 결정'],
      ['Vector DB', '미정·선정 필요', '정책·상품 속성·과거 승인 사례 검색 필요성 검증 후 결정'],
      ['Sentry', '애플리케이션 오류', '프론트·백엔드 예외와 사용자 영향 추적'],
      ['Elasticsearch / Logstash / Kibana', '로그 분석', '배치·권한·Teams 전달·계산 원장 탐색과 보존'],
      ['Prometheus / Grafana', '지표·대시보드', 'API 지연·배치 신선도·큐·외부 연동 성공률 관찰'],
    ],
  },
};

export default function TechStackPage() {
  const [activeTab, setActiveTab] = useState<StackTab>('all');
  const tabs: [StackTab, string][] = [['all', '전체'], ['frontend', '프론트엔드'], ['backend', '백엔드'], ['data', '데이터'], ['infra', '인프라·운영'], ['ai', 'AI·모니터링']];
  const visible = activeTab === 'all' ? Object.entries(stacks) : [[activeTab, stacks[activeTab]]];

  return <>
    <section className="page-hero"><div className="container"><Reveal><span className="eyebrow">03-C · Tech stack &amp; architecture</span></Reveal><Reveal><h1>4개 계열사 통합 서비스를 위한<br /><em>현재 기술 스택</em></h1></Reveal><Reveal><p>상품·재고·예약 capacity를 공통 모델로 연결하고, 권한·계산·승인·증빙을 분리해 운영합니다. 아래 목록은 현재 합의된 스택만 반영합니다.</p></Reveal><div className="formula-hero-points"><span><b>공통</b> React 19 · Java 17 · Oracle</span><span><b>검증</b> Playwright · Vitest · k6</span><span><b>관제</b> Sentry · ELK · Prometheus/Grafana</span></div><div className="affiliate-tabs" role="tablist" aria-label="기술 영역 선택">{tabs.map(([id, label]) => <button type="button" key={id} role="tab" aria-selected={activeTab === id} className={activeTab === id ? 'affiliate-tab active' : 'affiliate-tab'} onClick={() => setActiveTab(id)}><strong>{label}</strong><span>{id === 'all' ? '전체 아키텍처' : stacks[id].summary}</span></button>)}</div></div></section>
