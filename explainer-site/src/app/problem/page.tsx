...
const contexts = [
  ...
  { name: '현대리바트', tag: '가구·리빙', color: 'violet', title: '보관·배송·설치 비용이 중요합니다.', body: '보관일·배송·설치·파손·AS 비용을 함께 계산해야 할인이나 묶음 전략의 손익을 비교할 수 있습니다.', signal: '보관비 · 설치비 · 회수비' },
  { name: '현대그린푸드', tag: '식품·콜드체인', color: 'orange', title: '안전과 소비기한이 비용보다 먼저입니다.', body: '로트·소비기한·온도등급·검사 상태를 확인한 뒤 할인·추가입고·회수·폐기 대안을 비교합니다.', signal: '소비기한 · 온도 · 폐기비' },
];
...
export default function ProblemPage() {
  return <>
    ...
    <section className="section-tight band"><div className="container split"><div><span className="eyebrow">What changes</span><h2 className="section-heading" style={{marginTop: 14, marginBottom: 20}}>통합은 같은 결론이 아니라,<br />같은 언어를 갖는 것입니다.</h2><p className="lead">각 계열사 담당자는 자사의 비용·권한·정책으로 판단하고, 통합 운영은 위험 규모·처리율·<Link href="/glossary#incremental-profit" className="inline-link">증분이익</Link> 같은 공통 지표로 결과를 비교합니다.</p></div><div className="callout"><strong>우리가 만들지 않는 것</strong><p>통합 장바구니, 소비자 결제, 주문·배송 실행, 승인 없는 가격·수량 변경. 이 서비스는 쇼핑몰이 아니라 의사결정 지원 도구입니다.</p></div></div></section>
  </>;
}
