...
type AffiliateId = 'wellness' | 'livart' | 'greenfood';
...
const formulaHelps: Record<string, FormulaHelp> = {
  ...
  - 아무것도 바꾸지 않았을 때의 결과
    ...
    terms: [
      ...
      { symbol: 'AvoidedCost_s', meaning: '미리 처리해서 아낀 돈', detail: '보관비, 폐기비, 배송·파손비처럼 상품을 빨리 처리해서 줄어드는 비용입니다.', glossaryId: 'avoidable-cost' },
...
그 외에는 0`,
    terms: [
      ...
      { symbol: 'freshness_ok', meaning: '데이터가 최신인지', detail: '재고·가격·소비기한 정보가 오래되지 않았는지 확인합니다.' },
      { symbol: 'capacity_ok', meaning: '배송·처리할 여유가 있는지', detail: '배송, 설치, 냉장·냉동 공간이 부족하지 않은지 봅니다.', glossaryId: 'delivery-capacity' },
  ...
  max(0, 평소에 팔리던 수량
    ...
    × 예측을 얼마나 믿을 수 있는지))`,
    terms: [
      { symbol: 'Q_s', meaning: '전략 후 팔릴 것으로 보는 수량', detail: '할인이나 묶음판매를 적용했을 때 예상되는 판매 수량입니다.' },
      { symbol: 'Q_available', meaning: '실제로 팔 수 있는 최대 수량', detail: '판매 가능한 재고, 배송·설치 가능량처럼 현재 제공할 수 있는 한도입니다.' },
      { symbol: 'Q_base', meaning: '평소에 팔리던 수량', detail: '같은 상품이 같은 기간에 보통 얼마나 팔렸는지를 기준으로 삼습니다.', glossaryId: 'sales-velocity' },
      { symbol: 'F_time', meaning: '남은 시간의 영향', detail: '소비기한, 납기·설치일까지 남은 시간이 판매량에 미치는 영향입니다.', glossaryId: 'dday' },
...
이번 판매 때문에 더 드는 돈 = 예상 판매량 × (판매 수수료
  ...
  + 행사 준비에 드는 고정비`,
    terms: [
      ...
      { symbol: 'commission', meaning: '판매 수수료', detail: '판매채널이나 제휴사에 지불하는 수수료입니다.' },
      { symbol: 'payment', meaning: '결제 처리비', detail: '결제와 주문을 처리할 때 드는 비용입니다.' },
...
실패했을 때 예상되는 손해 = 실패 가능성 × 실패 손해`,
    terms: [
      ...
      { symbol: 'capacity_loss_avoided', meaning: '공간·처리 여유를 지켜 아낀 금액', detail: '창고·설치 슬롯·배송 공간을 비워 두지 않아 생긴 이익입니다.' },
...
등급 = 정상 / 주의 / 위험`,
    terms: [
      ...
      { symbol: 'w_k', meaning: '각 신호의 중요도', detail: '소비기한, 보관공간, 배송·파손 비용 중 무엇을 더 중요하게 볼지 정합니다.' },
...
const profiles: AffiliateProfile[] = [
  {
    ...
    keyInputs: ['SKU·브랜드·성분·기능·대상', '로트·소비기한·보관조건', '재고·주문·판매속도', '채널별 가격·쿠폰·배송'],
  ...
  },
  {
    ...
    riskSignals: ['소비기한 압박', '예상 폐기량×처리단가', '콜드체인 capacity 부족', '주문 마감과 배송일 불일치'],
...
export default function FormulasPage() {
    ...
    <>
      <section className="page-hero formula-hero">
        <div className="container">
          ...
          <Reveal>
            <h1>3개 계열사를 하나의<br /><em>판단 수식으로 연결합니다.</em></h1>
          </Reveal>
          <Reveal>
            <p>
      ...
      <section className="section formula-section">
        <div className="container">
          <div className="section-heading">
            ...
            <p>모든 상품은 “기준선보다 실제로 더 나아지는가”를 같은 방식으로 비교합니다. 각 카드 오른쪽 위의 <strong>용어 설명</strong> 버튼을 누르면 기호를 쉬운 말로 풀어볼 수 있습니다.</p>
          </div>
  ...
  ∧ data_quality_ok
              ...
              <p>하드 차단 조건이 0이면 이익이 큰 후보도 추천하지 않습니다. 식품의 소비기한·검사 상태, 리바트의 설치 슬롯, 웰니스 상품의 표시·보관정보가 여기에 들어갑니다.</p>
              <SourceLinks ids={['project-policy', 'food-label-standards']} />
            </div>
          </div>
        </div>
      </section>
  ...
  max(0, Q_base
    ...
    × confidence))`}</FormulaBlock>
              <p><code>Q_available</code>은 판매 가능한 실재고와 배송·설치 가능량 중 더 작은 값으로 제한합니다.</p>
            </article>
            <article className="formula-panel formula-panel-with-help">
...
VariableCost_s = Q_s × (commission
  ...
  + campaign_fixed_cost`}</FormulaBlock>
              <p>쿠폰·포인트·배송·설치·콜드체인·환불처럼 전략 때문에 변하는 현금만 분리합니다.</p>
            </article>
            <article className="formula-panel formula-panel-with-help">
...
RiskPenalty_s = probability × impact`}</FormulaBlock>
            <p>웰니스는 소비기한·표시·회수 비용, 리바트는 공간·설치·배송·파손 비용, 그린푸드는 소비기한·콜드체인·폐기 비용을 계열사 프로필에서 가져옵니다.</p>
            </article>
          </div>
          <SourceLinks ids={['markdown-paper', 'markdown-perishable', 'project-simulation']} />
        </div>
      </section>
      ...
      <section className="section formula-section">
        <div className="container">
          ...
          <div className="simulation-result">
            ...
            <p>승인된 전략은 revision과 데이터 cutoff를 함께 저장하고, 실제 판매 결과가 연결되면 예측 오차를 비교합니다.</p>
          </div>
      ...
      <section className="section-tight formula-section band">
        <div className="container">
          ...
          <div className="evidence-grid">
            <article className="evidence-card"><span className="pill">P0</span><h3>이번 MVP에서 계산</h3><p>3개 계열사 상품의 공통 목적함수, 위험점수, 계열사별 입력·비용·하드 차단, 출처 링크와 시뮬레이션 결과.</p></article>
