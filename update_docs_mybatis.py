...
| **MyBatis 3.5+ (단독 채택, JPA 미사용)** | **[단독 데이터 접근 계층]** 재고 CRUD, 다중 조건 동적 필터링, 대용량 손실 집계 리포트, Window Function/CTE SQL 등 전체 DB 처리 전담 | **vs JPA + QueryDSL / JPA 단독**<br>JPA와 SQL 매퍼 동시 도입에 따른 복잡도와 학습 곡선을 배제하고, 개발자가 SQL을 100% 직접 작성 및 제어·튜닝 가능. 동적 쿼리는 MyBatis `<if>`, `<choose>`, `<where>` 태그 활용 | XML 매핑 파일 작성 및 쿼리 관리 필요 (자바 오타 컴파일 체크 미지원) |
...
flowchart TB
    ...
    subgraph AppCluster [백엔드 애플리케이션]
        ...
        MyBatis[MyBatis 3.5 SQL Mapper - 단독 채택]
...
print("Updated docs/architecture-and-tech-stack.md with MyBatis Only")
