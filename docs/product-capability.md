# 4개 계열사 통합 재고 의사결정 서비스 기능·데이터·수식 명세

작성 기준: 2026-07-30

대상 계열사: 현대웰니스, 더현대트래블, 현대리바트, 현대그린푸드
...
계열사 담당자가 소속 계열사의 상품·재고·판매·capacity 정보를 확인하고, 계열사와 카테고리에 맞는 위험재고 처리 전략을 비교·시뮬레이션·승인할 수 있는 통합 운영 서비스다. 데이터와 화면은 통합하지만 상품 단위, 처리기한, 법규, 비용, 위험 가중치와 하드 차단 조건은 계열사별 정책으로 분리한다.

서비스의 최종 결과는 “AI가 임의로 가격을 바꾸는 것”이 아니다. 결정론적 정책·수식 엔진이 실행 가능 여부와 손익을 계산하고, 예측 모델이 판매·예약량을 추정하며, LLM은 승인된 결과를 사람이 이해하기 쉬운 전략 설명으로 변환한다. 최종 가격·수량·실행은 권한 있는 담당자가 승인한다.

## CONSTRAINTS

### 고정 정책

- 네 계열사의 데이터를 하나의 공통 조회 모델로 관리하되, 계열사·법인·점포·채널 권한은 분리한다.
- 원가는 일반 화면에 표시하지 않는다. 손익 계산과 감사 로그에는 원가 버전·출처·접근권한을 남긴다.
- 법규·소비기한·예약 마감·소유권·처리 capacity·필수 데이터 품질을 비용이 아닌 하드 차단 조건으로 처리한다.
...
- 공통: 전략 실행 가능성, 예상 판매·예약량, 매출, 변동비, 회피비용, 위험손실, 기준선 대비 증분이익, 승인·감사 상태.
- 계열사별: 재고 단위, 처리기한, capacity, 법규 필드, 비용 항목, 위험 신호, 가중치, 임계값, 허용 전략.
- 카테고리별: 가격 반응, 판매 속도 기준, 반품·폐기율, 보관 조건, 최소 잔여기한, 채널 정책.
- 상품별: 현재 수량·가격·기한·예약·비용·상태·판매 이력과 정책 버전.

### 데이터 준비 수준

- 공개 자료: 상품군·채널·법규·운영 특성의 근거로 사용한다.
- 내부 계약 필요: ERP/POS/WMS/예약 원천, canonical 상품키, 원가·수수료·할인 부담, 공급사 위약금·회수·폐기 계약, 실제 판매·예약 이력, 데이터 소유자와 갱신주기.
...
- 판매 차단 예: 리콜, 표시 누락, 소비기한·보관조건 미확인, 예약 약관 미확인, 주문제작 취소 제한 미확인.
...
- 원천 범위: ERP, POS, WMS, 예약 시스템, 판매 채널, 정산 시스템, 배송·설치 시스템, 품질·검사 시스템.
...
- 컬럼: 상품/offer, 계열사, 카테고리, 가용량, 판매속도, 남은 기한/출발일까지, 위험점수, 차단 사유, 마지막 갱신.
...
- 그래프: 기간별 판매량, 가격·할인, 재고/예약 잔량, 취소·반품, 기한 압박.
...
- 이유는 “소비기한 14일 남음”, “출발일까지 3일”, “설치 슬롯 부족”처럼 원천값과 함께 표시한다.
...
- 계열사별 금지 조합: 법규 위반, 할인 한도 초과, 예약 마감 이후 판매, 설치·콜드체인 capacity 초과.
...
- 검증: 수량 범위, 할인 상한, 기간·출발일·소비기한, 채널 가능 여부, capacity, 하드 차단.
- 변경 시 원본 추천값과 사용자 변경값을 구분한다.

### F-22 시뮬레이션 결과

- 결과: 예상 판매·예약량, 매출, 변동비, 예상 이익, 마진율, 소진기간, 행사 종료 잔량, 회피비용, 위험손실.
...
- 대상 단위: offer·좌석·객실·서비스 slot.
- 필수 입력: departure_at, booking_cutoff_at, capacity, supplier_id, cancellation_rule_id, fx_rate_snapshot, channel.
- 주요 비용: 발권·결제·상담, 제휴 수수료, 환율, 변경·재예약, 공급사 취소·위약금.
- 하드 차단: 예약 마감·공급사 규정·환불 조건·capacity 미확인.
- 규칙: 물리 재고의 소비기한·폐기비를 적용하지 않는다. 미판매 capacity, 출발 임박도, 위약금과 기회손실을 계산한다.
...
AvoidedCost_s = avoided_holding + avoided_disposal
              + avoided_supplier_penalty + avoided_capacity_loss
...
`M_baseline`은 단순히 “현재 가격 유지”가 아니다. 프로모션을 하지 않아도 실행 가능한 정상 판매·예약 유지, 회수, 기부, 폐기, 위약금 처리 등의 대안 중 계열사 기준으로 가장 유리한 결과를 frozen snapshot으로 저장한다.
...
가중치와 임계값은 계열사·카테고리·정책 버전별로 저장하고 실제 폐기·위약금·잔여재고·마진 손실 outcome으로 보정한다.
...
- `sales_event`, `reservation_event`, `inventory_movement`
...
- Travel: offer·supplier·departure·booking cutoff·capacity·cancellation rule·FX.
...
- 계열사별 필수 입력과 하드 차단
- 결정론적 위험점수·시뮬레이션 엔진
- 그린푸드 로트/소비기한 또는 트래블 출발일/capacity 중 대표 수직 슬라이스 1개 완성
- 계산 결과·버전·감사 로그
...
- 실제 판매·예약·정산 원천 없이 예측 오차와 AI 학습 효과를 확정하는 것.

## OPEN QUESTIONS

- 각 계열사의 ERP/POS/WMS/예약/정산 원천과 데이터 소유자는 누구인가?
