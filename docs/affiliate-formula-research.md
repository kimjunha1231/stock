...
`z_ik`는 계열사별로 정의한 0~1 정규화 신호다. 공통 신호는 처리기한 압박, 판매속도 부족, 재고가치, 수요 불확실성, 처리 capacity 부족이며, 등급 임계값과 가중치는 버전으로 저장한다. 점수보다 하드 차단이 우선한다.
...
`sku_id`, `model_id`, `option_set`, `on_hand_qty`, `reserved_qty`, `dimension`, `weight`, `warehouse_location`, `lead_time_days`, `install_required`, `install_slot_available`, `delivery_zone`, `damage_rate`, `return_rate`, `as_cost`, `list_price`, `sale_price`.
...
- 할인으로 수요가 늘어도 설치 슬롯이 없으면 `capacity_ok = 0`으로 처리한다.

#### 계열사별 위험 신호

```text
z_space = normalized(on_hand_qty × cubic_volume)
z_age = normalized(days_in_warehouse)
z_install = 1 - min(1, install_slot_available / expected_install_demand)
...
`sku_id`, `lot_id`, `product_type`, `on_hand_qty`, `reserved_qty`, `expiry_at`, `temperature_class`, `storage_condition`, `origin`, `traceability_id`, `order_cutoff_at`, `delivery_window`, `cold_chain_capacity`, `waste_rate`, `supplier_recovery_rate`, `list_price`, `sale_price`.

#### 전략에 반영할 비용·제약

- 소비기한·보관조건·HACCP/이력추적 관련 필드가 없으면 판매전략을 만들지 않는다.
- 냉장·냉동 포장, 피킹, 배송, 아이스팩/보냉재, 회수·폐기와 음식물류 처리 비용.
- 사전예약 상품은 주문 마감일과 실제 배송일을 분리해 수요와 capacity를 계산한다.
...
z_cold_chain = 1 - min(1, cold_chain_capacity / expected_delivery_demand)
```
...
- 현재 MVP에서는 교차 계열사 번들 판매와 고객용 공개 카탈로그를 P2로 둔다. 수식 페이지에는 확장식을 설명하되, 실행 버튼은 활성화하지 않는다.
