...
t4_data = [
    ("구분", "역할 및 책임"),
    ("김준하", "프로젝트 총괄 / 백엔드(Spring Boot 3.3, QueryDSL), 파이썬 수리 최적화 엔진 개발, DB 스키마 설계 및 시스템 통합 배포"),
    ("김영만", "프론트엔드 개발 / React 18/19, Zustand 전역 상태 관리, TanStack Table/Query 기반 B2B 오퍼레이션 타워 UI 및 시뮬레이션 화면 구현"),
    ("이주영", "데이터 파이프라인 & 배치 개발 / Spring Batch 5 + Quartz Scheduler 기반 매일 새벽 02시 위험재고 5단계 자동 탐지 배치 구축"),
    ("최우진", "오퍼레이션 검증 & 제약조건 / 더현대 서울 직매입 재고 Oracle Retail 5대 분석 지표 및 5대 하드 차단 제약조건 검증")
]
...
print(f"Updated docx with team members saved to {output_path}")
