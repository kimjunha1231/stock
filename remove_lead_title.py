...
t1_data = [
    ...
    ("멘토", "김영만 수석 (현대백화점그룹 본사 재고전략팀)"),
    ("멘티(교육생)", "김영만, 김준하, 이주영, 최우진")
]
...
t2_data = [
    ...
    ("프로젝트 기간", "2026.07.28 ~ 2026.09.03 (개발마감: 08.31 / 발표준비: ~09.03)"),
...
t3_data = [
    ...
    ("검증, 서비스 마감 및 발표 준비", "E2E 시나리오 테스트, 개발 마감(8/31) 및 설명 웹사이트(explainer-site) 배포, 최종 발표 자료 준비 & 시연 데모 구축(~9/03)")
]
...
add_p("* 방법론 : 빅데이터 분석 및 Agile 반복 개발 방법론을 채택하여 분석기획, 도메인/데이터 설계, AI/연산 엔진 및 백엔드 구축, 프론트엔드 통합, 개발 마감(8/31), 발표준비(~9/03) 순으로 6주차 프로젝트 수행", size_pt=9.5)
...
add_p("                    [ HEYDI Stock 프로젝트 개발팀 ]\n                                   │\n       ┌──────────────┬────────────┴────────────┬──────────────┐\n       ▼              ▼                         ▼              ▼\n[ AI 수리최적화 ]  [ 위험재고탐지배치 ]     [ 시뮬레이션워크벤치 ] [ 성과관제&가드레일 ]\n   (김준하)        (김영만)                 (이주영)           (최우진)\n\n                        멘토: 김영만 수석 MD", font_name="Courier New", size_pt=8.5, color_rgb=(0, 51, 102), space_after=10)

add_sub_h(" 2. 구성원 역할")
t4 = doc.add_table(rows=5, cols=2)
set_table_borders(t4, color="B0C4DE")
t4_data = [
    ("구분", "역할 및 책임 (풀스택 기능별 담당)"),
    ("김준하", "풀스택 개발 / 백엔드-파이썬 수리 최적화 마진 연산 엔진, OpenAI GPT-4o-mini API 연동, DB 정규화 스키마 설계 및 전체 시스템 아키텍처 통합"),
    ("김영만", "풀스택 개발 / 위험 재고 자동 탐지 모듈 & 새벽 대용량 배치 파이프라인(Spring Batch 5 + Quartz), 위험도 관제 UI 및 모달 화면 구현"),
    ("이주영", "풀스택 개발 / 60fps 실시간 파라미터 조율 시뮬레이션 워크벤치(Zustand + Recharts) & 시뮬레이션 백엔드 REST API 구축"),
    ("최우진", "풀스택 개발 / 실제 승인 및 실행 전략 성과 비교 관제 모듈(AI 예상 vs 실제 오차 분석) & 5대 하드 차단 제약조건 가드레일 구현")
]
...
# 7. Section V: Management Process (Detailed Timeline)
add_sec_h("Ⅴ. 관리 프로세스 계획")
add_sub_h(" 1. 세부일정 추진 계획 ")
t_sch = doc.add_table(rows=7, cols=7)
set_table_borders(t_sch, color="B0C4DE")

sch_headers = ["세부개발내용", "1주 (7/28~8/03)", "2주 (8/04~8/10)", "3주 (8/11~8/17)", "4주 (8/18~8/24)", "5주 (8/25~8/31)", "6주 (9/01~9/03)"]
for c_idx, text in enumerate(sch_headers):
    format_cell_text(t_sch.rows[0].cells[c_idx], text, size_pt=8.0, bold=True, color_rgb=(0, 51, 102), align=WD_ALIGN_PARAGRAPH.CENTER)
...
for r_idx, r_data in enumerate(sch_rows, start=1):
    for c_idx, val in enumerate(r_data):
        ...
        color = (204, 0, 0) if c_idx == 6 and "■" in val else ((0, 102, 204) if "■" in val else (51, 51, 51))
        bold = True if "■" in val else False
        format_cell_text(t_sch.rows[r_idx].cells[c_idx], val, size_pt=7.5 if c_idx == 0 else 8.0, bold=bold, color_rgb=color, align=align)
...
out_rows = [
    ("계획수립", "프로젝트 수행계획서", "07/30", "본 문서"),
    ("분석", "요구사항정의서 & 의사결정 정책서", "08/03", "Do Nothing 기준선 정의"),
    ("설계", "DB 스키마 설계서 & AI 최적화 수식 명세서", "08/10", "PostgreSQL / PuLP 연산"),
    ("개발", "백엔드 API & Python AI 엔진 & React B2B 프론트엔드", "08/24", "GitHub 소스코드 반영"),
    ("개발 마감", "E2E 시나리오 테스트결과서 & 사용설명 웹사이트", "08/31", "개발 마감 및 explainer-site 배포"),
    ("발표 준비", "최종 발표 자료, 시연 데모 및 프로젝트 완료보고서", "09/03", "최종 발표 및 시연 완료")
]
...
print(f"Updated docx without '총괄' title saved to {output_path}")
